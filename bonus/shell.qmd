...
you learned about
[globbing (selecting files) with the `*` wildcard](week03/w3b_shellfiles.html#wildcard-expansion-globbing).
...
and irreproducible.
So, yhis is a good example of a general task that you can apply your coding skills to.
...
To go step-by-step, I will start by showing how you could rename a single
file like you would when using a loop, which is using _variables_.
...
2. **Rename files in a batch**  
   ...
   - You don't need a special construction to paste strings together:
     you simply type `bam` after what will be the extension-less file name from the `basename` command.
