...
and irreproducible.
So, this is a good example of a general task that you can apply your coding skills to.
For example, maybe you have a few thousand photos you want to rename in
a systematic way?
...
but it would be tedious, error-prone, and irreproducible to do this by hand.
Therefore, let's use a loop.

### Renaming a single file using variables

To go step-by-step, you will first see how you could rename a single file
_inside_ a loop, which is using _variables_.
You will also make a _separate copy_ of the files in a separate directory,
just to be safe.

To start with, say that the original file name is contained in a variable:

```bash
oldname=results/alignments/sample01.sam
```

And you can also save the new name in a variable:
...
```bash
# This will not execute the 'mv' command, it just prints it!
# This technique is useful as a sanity check when using variables.
...
Looks good? Then remove `echo` and rename the file
(using the `-v` option to make `mv` report what it's doing):
...
Here's how you can loop over these files,
saving each file name (one at a time) in the variable `$oldname`:
...
Next, inside the loop, assign a new name for each file:
...
Now, build and check the renaming command:
...
Finally, actually create the renamed copies by removing `echo`:
