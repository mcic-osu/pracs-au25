...
This session introduces high-performance computing in general
and the Ohio Supercomputer Center (OSC) more specifically. 
...
you'll learn a lot more about most topics that we will touch on here.
Also, a [deeper dive into OSC will follow in week 5](../week05/w5a_osc.qmd).
...
OSC has **three main websites** --- we will nearly exclusively use the first here:
...
- PIs typically request OSC projects.
:::
...
Therefore, login nodes are meant only to do things like organizing your files and
creating scripts for compute jobs,
and are ***not*** **meant for any serious computing**,
which should be done on the next type of node, compute nodes.
...
(a "compute job").
The Slurm *job scheduler*, which you'll learn to use later in this course,
then assigns the requested resources:
