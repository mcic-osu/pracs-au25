---
title: "Unix Shell Basics - part II"
...
In this session, we will continue with ...

::: callout-important

While some of these commands may seem to do somewhat trivial tasks,
TBA

- Getting more comfortable with working in the shell
- Syntax is consistent, can generalize across commands
- Better to stick with the shell also for these operations instead of going
  back and forth
...
Then, it may always be convenient to list an entire dir's contents,
as a dir could contain many files.

Can you try to use `ls` to see the file size **only** for the 
...
Where were these files created?

Can you create a create a new file `test.txt` inside a new dir `test`?
...
```
```bash-out
touch: test/test.txt: No such file or directory
```

Instead, we should first create the new dir with `mkdir` and then create the
new file in there:
...
Just like with copying files in file browsers on your computer,
the copy can have a different name than the original file,
or the same name if it's copied to a different dir.
...
That is, its basic syntax is `cp <source> <destination>`.
...
**Importantly, like with any Unix command**, 
...
To accomplish this,
we could of course just repeat the original file name in the target argument:
...
If we only use the `.` shortcut that indicates our current working dir,
...
Moving files to a different dir and renaming files is fundamentally the same
operation: you are changing the path.
It is important to realize that when talking about files,
both its filename (e.g. `buzz2.txt`) and the dir it is inside
...
mv buzz_copy.txt data/buzz_copy99.txt
```

Finally, unlike `cp`, `mv` is recursive by default!
...
in their respective apps, and cannot be operated on with the below commands.
That is quite limiting, as such apps are not always available and problems may
also arise if a file was produced by a more recent version of the app than you have,
or a "too-old" version.

While their simplicity may seem limiting,
**plain-text files are generally preferable in reproducible science**.
Additionally, many common omics file formats,
like FASTA, FASTQ, and GFF, are plain-text.

Therefore, we will be learning a number of Unix commands to view (this section)
and process/summarize (next section) plain-text files in various formats.
...
The **`wc`** command is different from the previous commands,
all of which printed some or all of a file's contents.
...
Start by moving back into the `sandbox` dir:
...
Above, we provided the `head` and `wc` commands with an argument:
the path to a file to operate on.
...
::: {.callout-warning}
#### A not about file extensions (TBA)
...
#### `grep` tips _(Click to expand)_

- While not always necessary,
  it is a good habit to always use quotes around the search pattern.
...
::: {.callout-warning collapse="true"}
#### Don't redirect to the input file! _(Click to expand)_
...
### Covered in the chapter but not in today's session
...
We'll cover all of these over the next few weeks.
...
-   In all course weeks, we will be working in the Unix shell,
    though our focus in several cases will be on a specific tool,
    such as Git in week 4.

-   A couple of sessions that will be focused on further developing shell skills:
    - Week 3, Thu session: Managing files in the shell
