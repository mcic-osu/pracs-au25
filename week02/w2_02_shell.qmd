...
In this session, we continue with basics Unix shell commands,
...
Can you use `ls` to see the file size **only** for the 
...
2. Can you create a create a new file `test.txt` inside a new dir `test`?
...
::: {.callout-warning}
#### A note about file extensions (TBA)
...
### Covered in the CSB chapter but not in today's session
