...
- The "**working dir**" is whatever dir/folder you are currently located in.
  In a coding interface, like the Unix shell we'll learn after this,
  you are _always_ in a certain working dir
  (and the same is basically true in a file browser).

- On "Unix-like" (Mac and Linux - OSC uses Linux) operating systems,
  there is a single starting point to the directory structure: 
  the **root**, depicted as **`/`**.
  While this works slightly differently on Windows,
  directory structures have starting points there as well.
...
- The OSC dir structure is somewhat different from that of a regular computer.
  Our Home dir is not `/home/<username>` like in the schematic on the left
  (reflecting the dir structure of a Mac), but `/users/PASXXXX/<username>`.
...
while **relative paths** do, and won't work if you're elsewhere --- don't absolute paths sound better? What could be a disadvantage of them?
