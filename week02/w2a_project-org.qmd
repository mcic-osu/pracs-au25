...
In this session, we will cover the principles behind and practice of good
**research project documentation and file organization**.
Why is this needed? Proper project documentation and organization facilitates:
...
![Two research project folder hierarchies, nicely separated and self-contained.<br>(The `$HOME` folder represents your personal Home folder.)](../img/proj-dirs-1.svg){fig-align="center" width="60%" fig-alt="A diagram showing two research project folder hierarchies." .lightbox}
...
Here, we'll go into the _folder structure_, whereas the last section of this session
will go into the `.md` _documentation files_ that are also shown in the image above.
...
- Subfolders are mostly organized by and named after the software that produced
  the results.

- In some cases, other kinds of descriptive names may make more sense.
...
- Say that the MultiQC program was run multiple times.
  On the left, these runs are named after the run date, whereas on the right,
  the names describes what each run was for.
  Both of these options have pros and cons.
  For example, always using a date is simple and consistent but is less clear
  about the purpose of each folder.
  In general, there are reasonable alternatives to some of the recommendations
  presented in this session --- see also the box below.
...
- Several of these folders can be named differently, such as:
  - Some researchers prefer `analysis` over `results`
  - Some researchers prefer `src` ("source") over `scripts`
  
- Some recommendations (such as in the @buffalo2015 book)
...
::: columns
::: column
![Example dir structure on a Mac computer, from [O'Neil 2017](https://open.oregonstate.education/computationalbiology/chapter/the-command-line-and-filesystem/)](../img/oneil_filesystem_ex2.png){fig-align="center" width="95%" fig-alt="A diagram of an example dir structure on a Mac computer." .lightbox}
:::
::: column
...
#### {{< fa user-edit >}} In the above schematic on the left, what is the path to the file `todo_list.txt`? _(Click for the solution)_
...
::: {.callout-important collapse="true"}
#### {{< fa user-edit >}} In the examples above, how can the mere file name `todo_list.txt` represent a path? _(Click for the solution)_
...
This is good question that we'll get back to in [week 5](../week05/w5b_data-management.qmd).
:::
...
and we will extensively practice with that during the course.
