...
During this lecture, you will learn how to use *Markdown*,
which is the simple plain-text markup language we recommend using for documentation.
...
and to write Unix shell code throughout the course.
...
Very conveniently, VS Code takes a specific folder as a
...
{{< fa user-edit >}}  Switch to the personal dir within `/fs/ess/PAS2880/users`
...
We will start using this terminal next week!
For now, try to _resize panes_ (terminal vs. editor, and the side bar)
...
(**`*`**) will produce italic and bold characters, respectively:
...
Below, we'll try some Markdown syntax in the `markdown-intro.md` file you created earlier.

First off, note that when you save a file in VS Code with an `.md` extension,
as you have done:
...
That preview looks something like this:
