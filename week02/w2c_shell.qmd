...
You'll be using the Unix shell almost constantly throughout this course,
and we have several lectures devoted to learning it, starting with this one.
We'll start with a conceptual introduction and why it useful to learn the shell.
...
Unix-like or Unix-based operating systems include Linux and Mac but not Windows.

For scientific computing, Unix-based operating systems are generally preferable.
While not as familiar to the general public,
supercomputers like the Ohio Supercomputer Center (OSC),
and most servers throughout the world all run on Linux, specifically.

That said, don't be alarmed if you have a Windows laptop/desktop.
For the purposes of this course, your computer can run on any operating system,
because we will connect to OSC and do our work there.
...
Also related is the term **terminal**: the program/app/window that can run a shell.
While all these terms are clearly not synonyms, then,
...
- **Software needs**\
  ...
  Many of the external software that we use for such tasks are run in the shell
...
3. In the form on the page that appears, fill out the following:
   ...
   - "_Working Directory_": your personal dir in `/fs/ess/PAS2880/users`
     ...
     [this differs from what you did last time!]{style="color:#bb0000"})
...
- Code-formatted text between `<` and `>` (such as `<username>` above),
  is commonly used to indicate not literal code but something that needs to be
  or will be replaced by something context-specific
  (such as your personal user name in the `<username>` example).
...
As you've seen above, options are specified with a dash `-`
(or two dashes `--`, as you'll see later).
So far, you've only worked with the kind of options that are also called "**flags**",
_which change some functionality in an ON/OFF type way_, such as:
...
Finally, you can also _combine_ options and arguments:
