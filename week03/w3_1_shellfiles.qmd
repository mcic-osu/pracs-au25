...
We'll add a keyboard shortcut to send code from your editor to the terminal.
...
- Find `Terminal: Run Selected Text in Active Terminal`, click on it,
  then add the shortcut <kbd>Ctrl</kbd>+<kbd>Enter</kbd>^[
...
### Downloading our practice dataset

First, make sure you are in your personal dir within `/fs/ess/PAS2880/users`:
...
#### Don't pay attention to `ls` dir sizes _(Click to expand)_
The sizes that `ls -l`/`ls -lh` show for a directory unfortunately does not
correspond to the total size of the dir and its contents:
basically, you should only pay attention to file sizes.

You could use the following command to figure out a total dir size:
...
by specifying one or more dirs as arguments:
...
Let's say that you want to check the size of a single file.
Then, it may be not be convenient to list an entire dir's contents.
...
We will download that one separately!^[
It was too large to be included in the GitHub repository.
]
</details>
...
2. We used a bit of complicated relative path to create the `sandbox` dir.
...
1. Where were these files created? Can you check their sizes?
...
When I use `< >` around text in a line of code,
...
check the [advanced shell page](../ref/adv-shell.qmd) if you're curious.
