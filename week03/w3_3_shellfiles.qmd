...
In globbing, the `*` wildcard matches any number of any character
...
Therefore, glob expansion will work with any command that accepts (multiple)
paths, such as _copy_ (`cp` command), *move* (`mv`) or *delete* (`rm`).

Additionally, we can first check which files those commands will "see"
...
We'll talk about regular expressions in ? (TBA)
:::
...
while doing this with code is a very nice example of a general task that you
...
We will first have to learn about the **`basename` command**,
which removes any dir name that may be present in a file path,
