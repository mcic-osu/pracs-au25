...
focusing on those that work with files and dirs.
...
- **Start a VS Code session like you've done before**
  ...
  7. Check that your are in `/fs/ess/PAS2880/users/$USER`
     (recall: `$USER` is a variable that represents your username!)
...
- Icons to toggle **(wide/Primary) Side Bar** options:
  ...
  - Debugger
  - ***Extensions***: To install extensions (*next week*).
...
And like you saw with `cal`, options and arguments can be combined:
...
That's especially true once you learn how to select/specify may files at once
using shortcuts known as wildcards, as will later in this lecture.
:::
...
Three notes as a preamble to the `mv` command, in part a recap from before:

1. Moving files versus renaming them is fundamentally the same operation:
...
3. A mere file name like `monarch.txt` can also represent a path:
   it implies that the file is in your current working dir.
