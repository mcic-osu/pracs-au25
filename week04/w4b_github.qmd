...
Generally, sharing repositories online allows you to:
...
These two options are associated with separate URLs.
In other words, for everything on GitHub,
**there are separate SSH and HTTPS URLs, and you should make sure to make the correct one**.
Since you have set up SSH but not HTTPS authentication,
you must use URLs with the following format:
...
1. First, tell Git to add a "remote" connection with **`git remote`**,
   ...
   - **`origin`** --- the nickname for the connection
     (could be anything but usually called "origin" by convention). 
...
- To and from the **currently active repository *"branch"*** (default: `main`)^[
  To learn more about branches, see the [Git bonus material](../bonus/git.qmd).].

Because you only have one remote connection and one branch in this repo,
you can simply use the following to push from now on:
...
Next, click where it says `x commits`
