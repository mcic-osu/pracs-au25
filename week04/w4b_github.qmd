...
- We don't want to add README or `.gitignore`,
  since we already have those in our local repo.
  In general, to avoid confusion,
  it's best to not add or edit any files in your GitHub repo:
  do this locally, then upload to GitHub (as we'll see below).
...
In a single-user workflow (i.e., without collaborators),
all changes are typically made in the local repository,
...
consists at its cores of the following order of steps/commands:
...
5. Get an overview of your commits with `git log`
