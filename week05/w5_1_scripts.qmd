...
subtitle: "Week 5 - Shell scripts - Part I"
...
A key reason we need to use shell scripts rather than executing the programs
directly in the shell is that they make take quite a while to run,
and we often need to run these program many times (e.g., once for each sample).
...
3. Making these programs available at OSC (next week)
4. Submitting batch jobs with Slurm (in two weeks)
...
- *"Line actions"*:
  - <kbd>Ctrl/⌘</kbd>+<kbd>X</kbd> / <kbd>C</kbd> will
    **cut/copy the entire line** where the cursor is, when nothing is selected (!)
...
#### Make sure you have the "shellcheck" extension
...
You should have already installed it in week 2...
...
The quote above uses the word "Bash", and we'll see that term more often this week.
...
2. The script needs to be "executable".
   You can do that by changing the file permissions ---
   we will talk about file permissions next week.
...
When you execute a script, you can pass arguments to it,
...
In the next section,
we'll see how we can work with these arguments inside our scripts.
...
```bash
# Move out of the 'sandbox' dir (back to /fs/ess/PAS2880/users/$SUER/week04)
cd ..
```
