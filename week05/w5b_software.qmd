...
3. _Optional: Create a new Markdown file for class notes and save it in your `week05` dir._
...
2. Search for the program STAR^[
   ...
   - Another program `starccm` also matches the search term,
     so the output is rather long and will open in a `less`-like pager
     (press <kbd>q</kbd> to exit).
     
   - Two versions are available, the most recent one is `2.7.11b`.
...
3. Load the most recent of the STAR versions and test whether you can use it 
   by running `STAR`.
...
through OSC modules, we need an alternative way to get access to these.
...
- Once software is installed,
  using it is identical to using other software that has been installed.
  With containers, on the other hand,
  you essentially need a long "prefix" to your commands.
- It is a bit easier to create custom environments with multiple programs or
  with manually installed programs

Key advantages of containers are that:

- They are more portable (likely to work across different computers)
  and for that and a few other reasons, more reproducible than Conda
- It is quicker to download a container than to install software with Conda
- With the usage you'll learn, you need to remember fewer commands than with Conda
- At OSC, it doesn't require the loading of a module unlike with Conda
...
As such, the first step to using a container is finding/obtaining one.
...
This applies to both containers and Conda:
...
- You may run into an old or "low-key" program that is not available.
...
For example, if the command was `fastqc -v`:
...
_(To permamently change this location, you would need to make add_
