...
This session focuses on using shell scripts to run bioinformatics programs
with command-line interfaces (CLIs). You will learn:
...
Recall that you are learning about shell scripts to eventually be able to run
your analyses highly efficiently by submitting scripts as
...
We will continue to use **Markdown files** for this purpose.

We'll start with a quick intro to a first example of a CLI tool,
the FastQC program.
...
but we need to keep a record of our analysis steps,
which we will save in a Markdown file.
For small projects, a simple protocol could be added to your main `README.md`.
Let's practice with that by creating and writing a README for our current analysis.
...
In a few weeks, we'll discuss how you can organize this for larger projects.
:::
...
<details><summary>Click here for a minor hint</summary>
The output above has a blank line between the output for each file.
You can print a blank line by using `echo` without giving it any text to print.
</details>
...
#### Hard-coding settings _(Click to expand)_
