...
other-links:
  ...
  - text: "Optional reading: Advanced batch jobs"
    icon: book
...
(regardless of whether that is on a login or compute node).
...
But first, recall from last week how we have so far run Bash scripts:
...
and printed the output to the screen.
...
but typically, scripts have both, and we'll see examples of this below.
...
sleep 30s
echo "I'm awake! Successfully script sleep.sh"
```
...
- `PARTITION` --- The type of queue (usually auto-assigned and not of interest,
   unless you e.g. submit to a special node type)
- `NAME` --- The name of the job
- `USER` --- The user name of the user who submitted the job
- `STATE` --- The job's state, usually either `PENDING` (queued) or `RUNNING`.
  **Finished jobs do not appear**.
...
(`--account=PAS2880`) and short format (`-A PAS2880`).
...
- "Wall time" versus "core hours": if a job runs for 2 hour and used 8 cores,
  the wall time was 2 hours and the number of core hours was 2 x 8 = 16.

- The default time limit is 1 hour. Accepted time formats include:
...
- OSC bills you for the time your job actually used, not what you reserved.
  But jobs that request more time may be queued longer before they start.
...
- Keep the number of _tasks_ and _nodes_ to their defaults of 1
  (in which case the above `-c` option simply specifies the number of cores).

- Inform the program that you're running about the number of available cores ---
...
- The maximum amount of memory you can request on regular Pitzer compute nodes.
  ...
  `hugemem` nodes ---
  switching to such a node can happen automatically based on your requested amount,
  though with caveats: see
  [this OSC page for details on Pitzer](https://www.osc.edu/resources/technical_support/supercomputers/pitzer/batch_limit_rules).
...
- **Only `FAIL` for shorter-running jobs** (roughly up to a few hours)\
...
| Only allow job to begin after a specific time | `--begin=2024-04-05T12:00:00`
