...
- Moving the `garrigos_data` dir one level up, out of `week04`
  (since you'll keep using this data this and next week):
...
- Copying two scripts from last week which you'll use again:
  ...
  ```bash
  cp /fs/ess/PAS2880/users/$USER/week04/scripts/printname.sh scripts/
  cp /fs/ess/PAS2880/users/$USER/week04/scripts/fastqc.sh scripts/
  ```
...
But first, recall from last week that you can directly run a Bash script as follows:
