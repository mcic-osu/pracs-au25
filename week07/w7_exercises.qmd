...
2. Navigate to your dir `week07`
3. Create a file `exercises.md` for notes and answers
...
2. To check that the container works,
   ...
   and you'll also use this info below!

## Exercise II: Write a TrimGalore script

Write a shell script `scripts/trimgalore.sh`^[
Recommended full path: `/fs/ess/PAS2880/users/$USER/week07/trimgalore.sh`]
that runs TrimGalore on paired-end FASTQ files as follows:

- The script should run TrimGalore **once** per sample i.e. per two FASTQ files.
...
Next week, you'll submitting this script as a batch job,
and that will give you the opportunity to use multiple cores.
:::

## Exercise III: Run your TrimGalore script once
...
2. Check if everything went well by scanning the output that is printed to screen,
   ...
   _and describe your process in the `exercises.md`.)_
...
## Exercise IV: Run your TrimGalore script for multiple samples

1. In `exercises.md`, write and run a `for` loop that will run your TrimGalore
   script for multiple samples in the `../garrigos-data/fastq` dir.
   
   Because you are not submitting these scripts as batch jobs yet
   (you'll do that next week!), it will take too long to run the script for every file.
   Therefore, only loop over the 7 sample IDs that start with `ERR1080286`.
   And make sure you are still running the script once per sample!
   
2. Monitor the output printed to screen,
   and check the output files once it's done.

## Solutions

### Exercise 1

<details><summary>1 - The URL to the container</summary><p>

Currently, the most recent Trim-Galore version is 0.6.10:
