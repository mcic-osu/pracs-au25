...
- Slurm options to request specific resources for your compute jobs,
  and how to add these inside your script
...
(Omitting the `--account` option, as shown above,
is possible when you specify this option _inside the script_, as you'll see.)
...
- Output that is **printed to screen** when you directly run a script
  (`bash myscript.sh`),
  and that ends up in the **Slurm log file** when you submit the script as a batch job.
  ...
  "standard error" (error output) -- see the box in the
  [section on Slurm log files](#output-slurm-log-files) for more.
...
- Output of commands written to output files inside the script
  ...
  regardless of whether you run the script with `bash` or `sbatch`.
...
and/or you may be submitting many jobs at once.
...
- A Slurm log file with the script's standard output and standard error
  (i.e. output that is otherwise printed to screen).
