...
## The output streams _stdout_ and _stderr_, and separating them

The output that programs (including basic Unix commands) print to screen can
be divided into two types:

1. **Standard output** (_stdout_) for regular output
2. **Standard error** (_stderr_) for error messages and sometimes warnings messages

Most of the time, you can't tell the difference between these two,
but you can split them.
For example,
while both _stdout_ and _stderr_ by default end up in the same Slurm log file,
it is possible to separate them into different files.
You can do that by using the `--error` option, e.g. `--error=slurm-fastqc-%j.err`
(in that case, only _stdout_ will go into the file specified with `--output`).

Separating _stdout_ and _stderr_ like that could be useful because it may make
...
such that both _stdout_ and _stderr_ end up in that file.
:::
...
- If you _additionally_ didn't receive an email from Slurm,
  this implies the job succeeded -- job failure should have triggered an email
  due to the `#SBATCH --mail-type=FAIL` line.
...
Check for the presence of the main FastQC output files:
...
It can also be a good idea to pay attention to the file sizes, as shown by `ls -lh`.
...
files -- these should always be a red flag.
:::
...
Now, let's loop over all FASTQ files to submit a job for each file: 
