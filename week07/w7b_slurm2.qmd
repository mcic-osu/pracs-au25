...
### ...Or download a version of this script if you don't have it
```bash
wget -P scripts <TBA-URL>
```
:::

Next, add sbatch options with the following `#SBATCH` lines in the script:
...
We will also tell TrimGalore about the number of cores we have at our disposal
by including `--cores 4`:
...
Now, we are ready to submit the script.
Let's test it for one file first:
