...
- _Some_ results should also be uploaded to NCBI,
...
- Other results but also other kinds of data can (and is increasingly required to)
  be shared via repositories like [Dryad](https://datadryad.org/) and
...
Here are the specific guidelines for these agencies:

- [USDA NIFA guidelines](https://www.nifa.usda.gov/grants/lifecycle/pre-award/data-management-plans)
- [NSF guidelines](https://www.nsf.gov/funding/data-management-plan)
:::
...
and because of the amount of data on OSC, you may not want to either.
...
Data syncing is especially salient when for a given project,
you do analyses both at OSC and on your local computer.
To reduce this friction,
I recommend that for projects for which you need OSC,
you instead try to do _all_ your work there.
When you do so,
it is even OK to keep separate sets of files at OSC and your local computer --
for example,
you may only keep the manuscript and related files on your local computer,
...
but I chose to show you **FileZilla** because it works on all operating systems^[
...
1. **Install and open FileZilla**\
   ...
   click that, then install the program, and open it.
  
2. **Connect to OSC**\
   To connect FileZilla to OSC, find the **Site Manager**:
...
I would recommend to avoid downloading these to your own computer and then transferring
them to OSC,
but instead to **download them directly to OSC using commands**.
...
We actually need to do this because the `garrigos-data` repo contains only
...
![](../img/ncbi-ftp-dir.png){fig-align="center" width="95%" fig-alt="A browser screenshot of the NCBI FTP dir for the Culex pipiens genome" .lightbox}
...
2. Type `wget` and <kbd>Space</kbd> in your terminal, and paste the URL,
...
4. Move it to your `garrigos-data` dir:
   ...
   ```bash
   mv -v GCA_016801865.2_TS_CPP_V2_genomic.fna.gz garrigos-data/ref
   ```
...
- You can use `wget`s `-P` option to directly download to a different dir,
  ...
  ```bash
  URL=https://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/016/801/865/GCA_016801865.2_TS_CPP_V2/GCA_016801865.2_TS_CPP_V2_genomic.fna.gz
  wget -P data/ref "$URL"
  ```

- Another commonly used downloading command is `curl`.
  `wget` and `curl` have similar functionality,
  so you don't really need to learn both.
:::
...
1. **Read** permissions allow you to read and copy files/dirs
2. **Write** permissions allow you to move, rename, modify, overwrite, or delete
3. **Execute** permissions allow you to directly execute a file (see the box below)
...
- `+` : add a permission
- `-` : remove a permission
- `=` : set a permission to (regardless of its previous status)

These are used with both the kind of permission and the user category --
for example:
...
- To set read + write + execute (`rwx`) permissions for all (`a`):
  ...
  it should show up in **[green]{style="color:green"}** in your terminal.
  :::
