...
the hidden patterns, and trends that we cannot usually see easily in a raw data . In this
...
`data` function loads the sacrificed data sets and list of other available data sets.
...
::: {.callout-note}
# Penguin data sets : It consists body meaurement data of 3 species of penguins collected from 3 islands in the Palmer 
...
- What do you think will happen if you keep the `color = sepcies` outside of `aes()`. 
...
The order of the color will corresponds to the order of species in the legend. If you are unsure which color
to use for your plot, use the `colors()` to explore various available colors.

What if you have 10 treatments that you want to color? It requires lot of typing. R have several functions
...
### Bar plot 
It is one of the commonly used plot type.
It can be used to visualize relationship between the categorical variable and numerical variable such as mean or counts per group. 
...
### Box plot
We use box plot to visualize the relationship of numerical and categorical variable. We can 
