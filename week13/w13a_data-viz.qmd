...
our findings effectively. Visual representation of data helps us to see
the hidden patterns, trends of the variables present in data. In this
...
the development of graphics. Learning the graphics will help you to
...
represent the data points and so on. You can add as much as layers you want in the
...
- Create scatter plot, box plot using `geom_point()`, `geom_boxplot()`
...
manipulation, `tidyr` for data tidying and more. By loading the
...
`palmerpenguins` datasets, which contains data on penguin species, their
physical measurements, and other attributes. 

::: {.callout-note}
It is good practice to always install package directly in R console. Otherwise, everytime you render 
...
Let's load the penguin dataset, look at the data and its structure.
...
::: {.callout-note}
# Penguin dataset : It consists body meaurement data of 3 species of penguins collected from 3 islands in the Palmer 
Archipelago, Antarctica. The dataset contains 8 variables and 344 observations.
...
from RStudio provides a overview of the different types of plots that
...
2. `Mapping`: Aesthetic mappings defines how variables in the data are 
mapped to visual properties of the plot, such as x and y axes, 
colors, shapes, and sizes.
3. `Geometry (geom)`: It add layers to represent our data such as
...
by coloring penguins by `species`. To change the color by species, we need to map the `species` variable to color
inside the `aes()` function.
**When categorical variable is mapped to color, unique colors are assigned to each categories automatically by ggplot2. And the legend will be added to show the category that corresponds to each color.**
...
Again, we want to map the fourth variable `island`. Lets map `island` to different shapes
of the plot.
...
[Here](https://ggplot2.tidyverse.org/articles/ggplot2-specs.html) are several other aesthetic mapping 
...
1. Can you think how to color all the points to red color in the above graph?
...
Therefore, only the points are colored by species while the line is in default color (black).
...
`title` add title to the plot, `x` and `y` add labels to x-axis and y-axis respectively.
...
1. Boxplot: We use boxplot to visualize the numerical and categorical variable relationship. We can 
...
2. Can color the jitter points with the species of penguins?
...
Lets say, in the last box plot, we want to facet box plot by species:
...
In this lecture, we learned how to create different types of plots using `ggplot2` package in R.
