...
1. At <https://ondemand.osc.edu>,
   ...
   Alternatively,
   you can connect your local VS Code installation OSC's Pitzer cluster.
...
If your research involves standard types of omics data analysis,
you can likely use such a pipeline, which may save a lot of time and effort.
Moreover, these pipelines are often well tested and maintained,
so you can be sure that you are following best practices.
...
I think this is well worth covering because running such a pipeline is a bit
different and more involved than running a typical piece of bioinformatics software.
Additionally, when you know how to run one nf-core pipeline,
...
- So, the most recent version is `nextflow/25.04.6`.
  Load it and check the version:
...
with the pipeline.
Let's perform such a test-run,
also to become familiar with some basic syntax of the Nextflow command:
...
- The `nextflow` command has several subcommands,
  and we use `run` to run a pipeline
- We specify the pipeline to run as an argument to `nextflow run`, here:
  `nf-core/rnaseq`^[
...
- Each `process >` line indicates a specific process (_step in the pipeline_) that is being run.
  - For some of these, it says `5 of 5` way on the right,
    ...
    multiple jobs are being run in parallel (e.g., one per sample).
  - Other processes have `1 of 1`,
    indicating that only a single job is run (e.g., for preparing the reference genome).
  - In my example, one line says `retries: 2`,
  ...
  - The hexadecimal string at the start of each line (e.g., `[d2/97278b]`)
    ...
    the `work` sub-dir where this job's files are stored^[
...
There is also a dir called `work` --
by default, the location of Nextflow's "work dir" is `work` within your current
working dir
(from which you execute the `nextflow` command):
...
I will take a look at the files for the last process, MultiQC,
which had the following printed in the log when it finished:

```bash-out-solo
[fb/68d030] process > NFCORE_RNASEQ:RNASEQ:MULTIQC (1) [100%] 1 of 1 ✔
```

So, let's check the files in relevant dir in the work dir --
...
For the pipeline run with our real data, we need to start with some prep:
...
`nextflow run` command: Nextflow will automatically detect and process such a file.
...
- Open the newly created `nextflow.config` file in the VS Code editor
  ...
  we'll keep this file as simple as possible,
  specifying only our "executor" program (Slurm) and the OSC project to be used
  for Slurm job submissions:
...
we will submit the `nextflow run` command as a Slurm batch job via a shell script.
...
the inputs and outputs will be passed as arguments to the shell script:
...
- The `-profile` will now simply be `singularity`
...
- We use the `-ansi-log false` option to change the logging format from what we
...
which represent the inputs and outputs discussed above:
...
With all above-mentioned options, our final `nextflow run` command is:
...
Next, we will define variables with the arguments needed to run the script:
...
Run the above lines in your terminal to define the variables,
...
You might catch the pipeline when there are many more running jobs --
...
::: exercise
#### {{< fa user-edit >}} Exercise: check a `work` dir
...
Move into the `work` dir for the `SALMON_QUANT` job for sample `ERR10802867`.
...
Take a look at the files that are present in this dir,
then open and skim through the `.command.log` file which is Salmon's log file.
...
Remove the folder with the final outputs and resubmit the script.
What do you expect to happen?

<details><summary> _Click for the solution_ </summary>

First off, the only potential reason that any processed need to be rerun is that
...
- The **gene count table**
  ...
  This is what you use for downstream analysis such as differential expression
  analysis -- which you already did last week!
...
- Many columns are present and it can be usefult to click **"Configure Columns"**
